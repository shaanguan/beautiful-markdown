 ...
 *
 * Note: Math ($...$, $$...$$) IS captured here at high priority so other
 * inline extensions (notably `superscript` ^…^ and `subscript` ~…~) don't
 * eat the `^` / `~` characters inside formulas before KaTeX gets a chance
 * to render them. The extension emits the raw `$...$` text untouched
 * inside a no-op span so KaTeX's auto-render can still scan the text node.
 */
...
(function (root) {
  ...
  const obsidianExtensions = {
    extensions: [
      {
        // CJK emphasis fix: CommonMark's flanking rules reject closing ** / *
        // when preceded by Unicode punctuation (e.g. fullwidth colon ：) and
        // followed by a non-punct, non-space char (CJK ideographs). This is
        // extremely common in Chinese markdown: **粗体：**正文. Catch these
        // cases before the built-in emStrong tokenizer rejects them.
        name: "cjkStrong",
        level: "inline",
        start(src) {
          const i = src.indexOf("**");
        ...
        tokenizer(src) {
          const m = /^\*\*((?:[^*]|\*(?!\*))+(?!\*)[\p{P}\p{S}])\*\*(?=[^\s\p{P}\p{S}*])/u.exec(src);
          if (!m) return;
          return {
            type: "cjkStrong",
            raw: m[0],
        ...
        renderer(token) {
          return `<strong>${inlineInner.call(this, token)}</strong>`;
        }
      },
      {
        name: "cjkEm",
        ...
        renderer(token) {
          return `<em>${inlineInner.call(this, token)}</em>`;
        }
      },
      {
        // Math: capture $$...$$ and $...$ BEFORE superscript / subscript /
        // any other inline tokenizer can chew on the ^ / ~ chars inside the
        // formula. We don't render the math here — we emit the literal
        // $-delimited text so KaTeX auto-render (delimiters-based) still
        // picks it up after DOM injection. High priority ensures marked
        // tries this first; falsy returns let the text fall through as
        // plain prose when the pattern isn't a valid formula.
        //
        // Why: a doc like `$E = mc^2, a^2 + b^2 = c^2$` was rendered as
        // garbage because the superscript ext greedily matched `^2 + b^`
        // inside the math block, turning the rest into broken HTML.
        name: "math",
        level: "inline",
        priority: 10000,
        start(src) {
        ...
        tokenizer(src) {
          // Display math: $$...$$ (no inner $$). Multi-line OK.
          let m = /^\$\$([\s\S]+?)\$\$/.exec(src);
          if (m && m[1].length > 0) {
            return { type: "math", raw: m[0], text: m[0], display: true };
          }
          // Inline math: $...$ with classic restrictions to avoid eating
          // literal dollar signs (prices, etc.):
          //   - opening $ NOT followed by whitespace / digit-with-trailing-$
          //   - closing $ NOT preceded by whitespace
          //   - no $ inside, no newlines
          //   - inner content non-empty
          m = /^\$(?!\s)([^\$\n]*?[^\s\$])\$(?!\d)/.exec(src);
          if (m && m[1].length > 0) {
            return { type: "math", raw: m[0], text: m[0], display: false };
          }
          // Single $X$ (one char, no whitespace): also valid math.
          m = /^\$([^\s\$\n])\$(?!\d)/.exec(src);
          if (m) {
            return { type: "math", raw: m[0], text: m[0], display: false };
          }
        ...
        renderer(token) {
          // Emit the raw $-delimited text so KaTeX auto-render finds it.
          // Wrap in a span just so marked treats us as an inline producer;
          // KaTeX walks text nodes and the wrapper is transparent.
