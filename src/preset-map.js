...
(function (root) {
  ...
  /* Presets own visual identity, not reading geometry. Keeping this boundary
   * at the compiler prevents bundled and custom presets from silently
   * changing the document rhythm before typography.css gets a chance to run. */
  const LOCKED_READING_KEYS = new Set([
    "font-text-size",
    "line-height-normal",
    "p-spacing",
    "heading-spacing",
  ...
  function compilePreset(preset) {
    ...
    for (const rawKey of Object.keys(preset)) {
      ...
      // Layout metrics are application policy, never preset data.
      if (LOCKED_READING_KEYS.has(varName)) continue;
  ...
  root.BaselinePreset = { compilePreset, presetToCSS, PREFIX, KEY_TYPES, LOCKED_READING_KEYS };
